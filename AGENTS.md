...
1. **優先參考台灣行情**：搜尋同款商品在台灣的售價（直營門市、選物店、蝦皮商城、PChome、momo 等），取中位數，定價比台灣行情低 10-25%
2. **無台灣參考價時**：才用干森費率公式試算（見 `sourcing/shipping-costs.yaml`），確保毛利率 ≥ 35%
3. **毛利自動判斷線：20%**：
   - 毛利率 ≥ 20% → 自動過關，繼續下一步，不回頭質疑 SKU
   - 毛利率 < 20% → 回報操作者，由操作者決定是否繼續或換 SKU
   - 步驟 ① 是**執行步驟**不是決策關卡，調研完成就往下走，不在這裡卡關
4. **所有台灣價格來源必須存檔**：每個產品在 `sourcing/price-refs/` 目錄下建立 `<product-slug>.md`，記錄台灣價格來源 URL、售價、查價日期。操作者可據此認證定價合理性
5. **所有日本官方產品頁面連結必須存檔**：同上檔案，記錄日本官網商品頁面 URL（操作者下單用）
6. **成本結構永不公開**：干森費率、運費公式、D-3 計算、進價（cost_jpy）僅存在內部檔案（`sourcing/shipping-costs.yaml`、`sourcing/price-refs/`、product.yaml 的非公開欄位），絕不寫入任何 Shopify 公開欄位（title、description、SEO、FAQ、JSON-LD、metafields 的 storefront-visible namespace）
...
③ 抓取產品圖片（從日本官網商品頁面）
   — 用步驟①記錄的日本官網 URL，curl 下載頁面 HTML 原始碼
   — 搜尋頁面中的 JSON 產品資料（含 images 和 cut_images 物件），取得真正產品圖檔名
   — 圖片通常放在官網 CDN：/storage/products/images/origin/<product_code>_<color>.webp
     和 /storage/products/cut-images/origin/<product_code>_<seq>_<color>.webp
   — curl 下載 origin 尺寸到 products/<product-slug>/images/（photo-01.webp ...）
   — 至少 6 張：主圖（黑色正面）、模特正面、模特背面、開襟展示、面料細節、收納展示
   — ⚠️ 不要直接抓 img tag 中的圖片（會拿到 UI 元件而非產品圖），要從 JS 資料中找
...
⑤ Claude 補完 AI 區段
   標題、描述、SEO、alt text（根據實際下載的圖片內容撰寫）、FAQ、schema
